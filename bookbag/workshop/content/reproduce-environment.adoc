...
In this serie if their body dies, their stack can be stored indefinitely. Their stacks can be downloaded into 
...
$ oc set env deployment/web \
    COOLSTORE_GW_ENDPOINT=http://istio-ingressgateway-istio-system.{APPS_HOSTNAME_SUFFIX}/staging-project{USER_ID} \
